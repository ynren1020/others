##########################################################################
#################Heatmap for yuanguo wang 2019-02-27######################
##########################################################################
...
#calculate Z-score by each gene(row)## second
...
#calculte z-score by all genes and samples## USE THIS ONE! (first)
#AFTER LOG TRANSFORM, THE VARIATION WAS GREATLY REDUCED AND HEATMAP LOOKS BETTER##
avg<-sum(dat)/(5*6) #1.909
...
tiff(filename = "yuanguo.heatmap_logtransform_zscore(sample_genes).tiff",width = 4.5, height = 3,units='in',res = 400,pointsize = 4)
H<-pheatmap(zscore,
            show_colnames = T,
            show_rownames = T,
            fontsize_row=12,
